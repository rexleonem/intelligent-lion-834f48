...
var Kelsey = function(){
	...
	var fileInput = function(){
		...
		jQuery(document).on('change', '.btn-file :file', function() {
			...
			var	label = input.val().replace(/\\/g, 'https://kelsey.dexignzone.com/').replace(/.*\//, '');
		...
		jQuery(window).on('scroll', function () {
			if(jQuery('.sticky-header').length > 0){
				...
				if ($(window).scrollTop() > menu.offset().top) {
					...
					$('.header-style-5 .container > .logo-header .logo').attr('src','https://kelsey.dexignzone.com/xhtml/error-404.html')
				}
			}
		});
